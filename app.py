...
def show_song(song_id):
    ...
    playlists = song.related_playlists
...
def add_song_to_playlist(playlist_id):
    ...
    song_choices = [
        (song.id, f"{song.title} - {song.artist}")
        for song in Song.query.filter(Song.id.notin_(curr_on_playlist))
    ]
    ...
    if form.validate_on_submit():
        ...
        selected_song = Song.query.get_or_404(selected_song_id)

        new_song = PlaylistSong(playlist_id = playlist.id, song_id=selected_song.id)
